...
For trash wheel data, the dimension of the dataset (exluding NA) is `r dim(trash_wheel_data)` which shows that there are 216 observations. Some key variables include weight (in tons), plastic bottles, and number of homes powered. The total weight tons collected from 2014 to 2017 is `r sum(trash_wheel_data$weight_tons)`, total number of plastic bottles collected is `r sum(trash_wheel_data$plastic_bottles)`, and total number of homes powered is `r sum(trash_wheel_data$homes_powered)`. The median number of sports balls in a dumpster in 2016 is `r trash_2016 = filter(trash_wheel_data, year == 2016)` `r median(trash_2016$sports_balls)`
